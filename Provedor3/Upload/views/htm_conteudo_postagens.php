...
if($conteudo_config->formato == 1){
	...
	<style type="text/css">
		...
		#section-postagens-<?=$conteudo_id?> {
			padding-bottom:40px !important;
			padding-top:60px !important;
		}

		#section-postagens-<?=$conteudo_id?> .owl-prev{
			top:100% !important;
			left:15px !important;
		}
		#section-postagens-<?=$conteudo_id?> .owl-next{
			top:100% !important;
			right:15px !important;
		}
		
		.carrosel_menu_post_div{
			text-align: center;
			margin-top:10px;
			padding-bottom:40px;
		}
		#section-postagens-<?=$conteudo_id?> .owl-carousel .owl-nav.disabled, .owl-carousel .owl-dots.disabled{
		...
		<?php
		if( ($conteudo_config->formato == 1) AND ($conteudo_config->itens_por_linha == 6) ){
			?>
...
<div id="section-postagens-<?=$conteudo_id?>" class="animate-effect" style="padding-top:40px; padding-bottom:30px; background-color: <?=$cores[46]?>; font-family: <?=$conteudo_config->font_family?>; ">
	...
	<?php if($conteudo_config->mostrar_titulo == 1){ ?>
		...
		<div class='row' >
			<div class='col-xs-12 col-sm-12 col-md-12' >
				<div>
					<h2 class="titulo_padrao" style="color:<?=$cores[47]?> !important; border-color:<?=$cores[47]?> !important; " ><?=$conteudo_config->titulo?></h2>
	...
	if(
			...
			<div class='col-xs-12 col-sm-3 col-md-3' >
				...
				if($conteudo_config->mostrar_categorias == 1){
					...
					<div>
						...
						<h2 class="titulo_padrao" style="padding-top: 30px; text-align: left; color:<?=$cores[47]?> !important;" >Categorias</h2> 
		...
		foreach ($postagens_lista as $key => $value) {
			...
			if($itens_listados <= $conteudo_config->max_itens){

				$endereconoticia = DOMINIO.$controller."/leitura/id/".$value['endereco'];

				if($conteudo_config->formato == 1){

					if($inicia_carrosel == 0){
						echo "<div class='owl-carousel postagens_destaques_".$conteudo_id."' >";
						$inicia_carrosel++;
					}

					echo " 
					<div class='item $classes_css'>
					<div class='noticias_1_div' style='background-color:".$cores[158]."'>
					...
					<a class='noticias_1_lermais' href='".$endereconoticia."' style='color:".$cores[159].";' >Leia mais</a>
		...
		<div class="carrosel_menu_post_div">
			<div class="carrosel_menu_post" style="color:<?=$cores[47]?> !important; cursor: pointer;" onclick="window.location='<?=DOMINIO?>blog';" style="cursor:pointer;" ><i class="fas fa-search" style="color:<?=$cores[47]?> !important;"></i> Ver Todas as Matérias</div>
		</div>
