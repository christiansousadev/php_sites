...
class usuarios extends controller {
	...
	public function alterar_grv(){
		...
		// executa
		$db = new mysql();
		$db->alterar("adm_usuario", $dados, " codigo='$codigo' ");			 
		...
		foreach ($lista_setores_todos as $key => $value) {
			...
			if( $this->post('setor_'.$value['id']) ){
				...
				if($confere){
					...
					$db = new mysql();
					$db->apagar("adm_setores_usuario", " usuario='$codigo' AND setor='".$value['id']."' ");
	...
	public function apagar_varios(){
		...
		foreach ($lista as $key => $value) {
			...
			if($this->post('apagar_'.$value['id']) == 1){
				...
				$db = new mysql();
				$db->apagar("adm_usuario", " codigo='".$value['codigo']."' AND codigo!='1' ");
