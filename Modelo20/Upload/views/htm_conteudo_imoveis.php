...
<div id="section-imoveis-<?=$conteudo_id?>" class="container-fluid animate-effect" style="background-color: <?=$cores[123]?>; font-family: <?=$conteudo_config->font_family?> !important; padding-left: 0px; padding-right:0px;">
...
<div class="imoveis_lista_normal_<?=$conteudo_id?>" > 
	...
	foreach ($lista_imoveis as $key => $value) {
		...
		if( ($itens_listados <= $conteudo_config->max_itens) OR ($conteudo_config->max_itens == 0) ){
			...
			echo "
			<div class='imovel_quadro $classes_css' >

			<div class='imovel_titulo' onClick=\"window.location='".$endereco_imo."';\" >".$value['titulo']."</div>

			<div class='imovel_imagem_div'>
			".$valor_final_imo."
			<div class='imovel_imagem hvr-grow ".$classes_css_img."' style='background-image:url(".$value['imagem'].");' >
	...
	foreach ($lista_imoveis as $key => $value) {
		...
		if( ($itens_listados <= $conteudo_config->max_itens) OR ($conteudo_config->max_itens == 0) ){
			...
			if($value['area_total']){
				echo "
				<span class='imoveis_lista_itens'>
				<img src='".LAYOUT."img/icos_imo/ico_areat.fw.png'  >
			...
			if($value['quartos']){
				echo "
				<span class='imoveis_lista_itens'>
				<img src='".LAYOUT."img/icos_imo/ico_quart.fw.png' >
...
<div style="position: absolute; bottom:55px; text-align: center; z-index: 9999; width: 70%; left: 15%;">
	<div class="carrosel_menu" onclick="window.location='<?=$link_ver_mais_imoveis?>';" >
</div>
