...
<section id="section-planos-<?=$conteudo_id?>" class="animate-effect" style="padding-top:50px; padding-bottom:80px; background-color: <?=$cores[44]?>; <?=$imagem_fundo?> ">
	<div class="container">
		...
		<?php if($conteudo_config->mostrar_titulo == 1){ ?>
			...
			<div class='row' >
				<div class='col-xs-12 col-sm-12 col-md-12' >
					<div>
						<h2 class="titulo_padrao" style="color:<?=$cores[45]?> !important; border-color:<?=$cores[45]?> !important;font-size: 48px; " ><?=$conteudo_config->titulo?></h2>
		...
		foreach ($planos_lista as $key => $value) {
			...
			<div class='planos_valor' style='color:".$cores['82']."; line-height:24px; ' >R$</span>".$value['valor']."</div>
