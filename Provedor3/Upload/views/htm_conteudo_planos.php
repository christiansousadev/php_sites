...
<div id="section-planos-<?=$conteudo_id?>" class="container-fluid animate-effect" style="padding-top:50px; padding-bottom:80px; background-color: <?=$cores[44]?>; "> 
	...
	<?php if($conteudo_config->descricao){ ?>
		...
		<div class='row' >
			<div class='col-xs-12 col-sm-12 col-md-12' >
				<div style="margin-top:-35px; padding-bottom:20px; font-size: 19px; color:<?=$cores[45]?>; text-align: center;">
	...
	foreach ($planos_lista as $key => $value) {
		...
		if($conteudo_config->plano_valor == 1){
			echo "
			<div class='planos_valor' style='color:".$cores['82']."; ' ><span style='font-size:14px;'>R$</span>".$value['valor']."<span style='font-size:14px;'>Mês</span></div>
			";
		}
